...
public class Main {
	...
	ArrayList<edge>adj[] = new ArrayList[nodes+1];
	for(int i = 0; i<=nodes; i++)adj[i] = new ArrayList<edge>();
	for(int i = 0; i<edges; i++){
	    int a = in.nextInt(), b = in.nextInt(), w = in.nextInt();
	    adj[a].add(new edge(b,w));
	    adj[b].add(new edge(a,w));
	}
	int[]dis = new int[nodes+1], dis1 = new int[nodes+1];
